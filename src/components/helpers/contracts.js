...
import { bonvoPropertyContractAddress, bonvoEscrowContractAddress, bonvoTokenContractAddress, userReputationContractAddress } from "../../utils/constants";
...
export function getBonvoEscrowContract(signer) {
    if (signer) {
        const bonvoEscrowContract = new ethers.Contract(bonvoEscrowContractAddress, bonvoEscrowContractABI, signer);
        return bonvoEscrowContract;
    } else {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const bonvoEscrowContract = new ethers.Contract(bonvoEscrowContractAddress, bonvoEscrowContractABI, provider);
        return bonvoEscrowContract;
    }
}
